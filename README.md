...
# 본인이 쓴글만 삭제 및 수정하기
1. 삭제 및 수정에 요청자와 글쓴이가 같은지 확인하여 해당 기능 수행하기
2. 글쓸때 글쓴이 정보도 같이 db에 저장하기
    > ```getServerSession(req, res, authOptions)``` 서버에 기입할 경우 요청과 응답도 함께 써주어야 한다.

# 회원가입 기능만들기
1. ```AuthSession.tsx``` 파일을 통해서 확인

# 댓글기능 만들기
1. ```detail/[id] page.tsx``` 에서 클라이언트 사이드 렌더링으로 작업

# 로딩파일

1. 사용될 페이지 안에 ```loding.tsx``` 를 생성
2. 그 페이지안에서 먼저 불러온 후 ```page.tsx```를 실행한다.
3. 
    <pre>
    ...
    - 페이지를 띄우기전에 먼저 위의 ```<Suspense>``` 로 자동으로 바꾸어 준다. 
    - loding.tsx를 만들면 자동으로 Suspense를 만들어서 실행하게 된다.
    - 인터넷이 느리거나 서버상 문제가 생길시 페이지 이동시 1~2초의 딜레이가 생길 경우 로딩중 ui를 만들어 준다.

# 에러페이지 만들기

1. 서버가 죽거나 db가 죽었을 경우 만들기
2. 에러페이지를 만들경우 if문으로 감싸거나 try-catch문으로 생성한다.
3. ```error.tsx``` 파일을만듬
4. 페이지에서 에러가 나면 3의 파일을 띄워준다. (에러페이지는 클라이언트 컴포넌트로 만들어야한다. ```'use client'```)


# 404 페이지

1. ```app``` 하위폴더로 ```[...not_found] > page.tsx```를 생성한다.
2. 해당폴더안에 ```notFound()``` 를 호출하는 컴포넌트를 만든다.
3. app 하위폴더로 ```not-found.tsx``` 생성하여 원하는 ui로 커스텀한다.
4. 에러페이지가 뜨기전에 해당 페이지가 먼저 뜨게 된다.
...
# 미들웨어
- 나의 서버기능을 여러개의 서버 API에 동시에 적용하고 싶은 경우
- 요청이랑 응답 사이에 간섭하는 코드
- root에 ```middleware.ts```를 만들어 준다.
- 로그인 정보시 NEXTAUTH_SECRET 키는 ```[...nextauth]```에서 사용한 시크릿 키(jwt 암호화 키)와 동일한 키를 사용한다.

# Server actions 기능

- ```"next": "13.4.0"``` 버전 이상에서 사용가능
- ```next.config.js```에서 14버전 아래일 경우 ```serverActions:true```를 기입한다. (안돼서 추가했음)
- 테스트를 위하여 write2 폴더 생성 -> ```page.tsx``` 생성
- ```'use client'```를 사용할경우(클라이언트 컴포넌트) 에서는 서버액션을 쓰면 위험하므로 따로 ```action.ts``` 파일을 만들어 export하여 갖다 쓴다.
- 글쓰기, 수정 , 삭제 기능 모두 서버액션을 이용하여 작성할 수 있다.